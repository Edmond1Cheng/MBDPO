...
class WorldModel(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self._F = layers.mlp(
            cfg.latent_dim + cfg.action_dim + cfg.task_dim, 2 * [cfg.mlp_dim], 1
        )
        ...
        self._score = layers.mlp(
            cfg.latent_dim
            + cfg.task_dim
            + cfg.horizon * cfg.action_dim
            + cfg.diffusion_time_embed_dim,
            2 * [cfg.mlp_dim],
            cfg.horizon * cfg.action_dim,
        )
    ...
    def _time_embed(self, tau, dim, device):
        half = dim // 2
        freq = torch.exp(
            torch.arange(half, device=device, dtype=torch.float32)
            * (-py_math.log(10000.0) / max(half - 1, 1))
        )
        angles = tau.float().unsqueeze(-1) * freq.unsqueeze(0)
    ...
    def score(self, z, a_tau, tau, task):
        """Predict diffusion score for noisy action sequence a_tau."""
        if z.ndim == 1:
            z = z.unsqueeze(0)
        if a_tau.ndim == 2:
            a_tau = a_tau.unsqueeze(0)
        if self.cfg.multitask:
            z = self.task_emb(z, task)
        a_flat = a_tau.reshape(a_tau.shape[0], -1)
        t_emb = self._time_embed(
            tau.view(-1), self.cfg.diffusion_time_embed_dim, a_tau.device
        )
        inp = torch.cat([z, a_flat, t_emb], dim=-1)
        out = self._score(inp)
        return out.view(a_tau.shape[0], self.cfg.horizon, self.cfg.action_dim)
